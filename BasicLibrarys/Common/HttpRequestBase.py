...
class HttpRequestBase:
    ...
    async def aio_requests(self, method, url, codec=False, data=None):
        async with aiohttp.ClientSession() as session:
            try:
                async with session.request(method, url=url, params=self.param, data=data, headers=self.__headers,
                                           timeout=self.timeout, allow_redirects=self.redirect) as res:
                    if res.status == 200:
                        content = await res.content.read()
                        code = chardet.detect(content)["encoding"]
                        if code is not None:
                            if codec:
                                text = content.decode(encoding="gbk", errors="ignore")
                            else:
                                text = content.decode(encoding=code, errors="ignore")
                            del res, content
                            return text
                        else:
                            text = content.decode(encoding="utf-8", errors="ignore")
                            del res, content
                            return text
                    else:
                        return self.requests_request(method, url=url, data=data)
            except aiohttp.client_exceptions.ClientResponseError or UnicodeDecodeError:
                return ""
            except aiohttp.client_exceptions.ClientConnectionError or aiohttp.client_exceptions.ClientPayloadError:
                return ""
